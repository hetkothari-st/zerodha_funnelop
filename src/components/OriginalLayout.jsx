...
import { Bell, ChevronDown, Check, Trash2, X, PanelLeftClose, Zap, LogOut } from 'lucide-react';
...
// ─────────────────────────────────────────────────────────────────────
// DEPTH CARD — Event-Driven Architecture
//
// Each card manages its OWN depth state via direct WebSocket event
// subscription. This completely bypasses the React render cascade:
//   WebSocket packet → depthEvents EventTarget → DepthCard local state
//
// The parent (OriginalLayout) NEVER re-renders for depth data updates.
// Only the individual card whose token received new data re-renders.
// ─────────────────────────────────────────────────────────────────────
const DepthCard = memo(({
    item,
    depthEvents,
    depthDataRef,
    isEditorOpen,
    activeStrikeEditor,
    setActiveStrikeEditor,
    onRemoveToken,
    onUpdateTokenQty,
    handleUpdateStrike
}) => {
    // Local depth state — seeded from ref, updated by events
    const [depth, setDepth] = useState(() => {
        const ref = depthDataRef?.current;
        return ref?.[item.tkn] || ref?.[String(item.tkn)] || null;
    });
    // Subscribe to depth-packet events for THIS token only
    useEffect(() => {
        // Seed with latest buffered data when token changes
        const ref = depthDataRef?.current;
        const initial = ref?.[item.tkn] || ref?.[String(item.tkn)];
        if (initial) { setDepth(initial); }

        if (!depthEvents) return;

        const tknStr = String(item.tkn);
        const handler = (e) => {
            const pkt = e.detail;
            if (String(pkt.Tkn || pkt.Token) === tknStr) {
                setDepth(pkt);
            }
        };
    ...
    }, [depthEvents, item.tkn, depthDataRef]);

    return (
        <div className="bg-[#0f1115] p-4 min-w-[280px] rounded-2xl shadow-xl relative group transition-all duration-500 border border-white/10">
            <button
            ...
            <div className="flex justify-between items-start mb-3">
                <div className="relative strike-editor-container">
                    <button
                        ...
                        onClick={() => setActiveStrikeEditor(isEditorOpen ? null : { id: item.id, strike: item.strike, index: item.index, focusIndex: 5 })}
                        onKeyDown={(e) => {
                    ...
                    >
                        ...
                        <ChevronDown size={14} className={cn("transition-transform", isEditorOpen && "rotate-180")} />
                    </button>
                ...
                <div className="flex flex-col items-end gap-0.5">
                    <span className="text-white/20 text-[8px] font-mono">{item.tkn}</span>
                </div>
            </div>
    ...
    // Memo only checks parent-controlled props.
    // Depth data is internal state (updated via events), so it's not checked here.
    return prev.item === next.item
        && prev.isEditorOpen === next.isEditorOpen
        && prev.activeStrikeEditor === next.activeStrikeEditor
        && prev.onRemoveToken === next.onRemoveToken
        && prev.onUpdateTokenQty === next.onUpdateTokenQty
        && prev.handleUpdateStrike === next.handleUpdateStrike;
});

// ─────────────────────────────────────────────────────────────────────
// LOG ROW — Memoized, re-renders only when log changes or timer ticks
// ─────────────────────────────────────────────────────────────────────
const OriginalLogRow = memo(({ log, showRelativeTimer, timeTick }) => {
    ...
    return prev.log.id === next.log.id &&
        prev.showRelativeTimer === next.showRelativeTimer &&
        (prev.showRelativeTimer ? prev.timeTick === next.timeTick : true);
});

// ─────────────────────────────────────────────────────────────────────
// LOGS TABLE — Memoized, isolated from depth data updates entirely
// ─────────────────────────────────────────────────────────────────────
const LogsTable = memo(({ logs, showRelativeTimer, setShowRelativeTimer, timeTick, showAllPrices, setShowAllPrices, logsHeight, handleResizeStart }) => {
    return (
        <div
            className="bg-[#0f1115] border border-white/10 rounded-2xl shadow-xl flex-shrink-0 flex flex-col p-0 overflow-hidden relative"
                    ...
                    <tbody className="text-xl font-medium">
                        {logs.slice(0, 150).map(log => (
                            <OriginalLogRow
                                key={log.id}
                                log={log}
                                showRelativeTimer={showRelativeTimer}
                                timeTick={timeTick}
                            />
                        ))}
                    </tbody>
...
// ─────────────────────────────────────────────────────────────────────
// ORIGINAL LAYOUT — Main Component
//
// Key change: does NOT receive `depthData` state as a prop.
// Instead receives:
//   - depthEvents: EventTarget for live packet subscription
//   - depthDataRef: React ref to latest buffered data (for initial card data)
//
// This means OriginalLayout ONLY re-renders when:
//   - logs change (new alerts)
//   - monitoredTokens change (user adds/removes)
//   - visibleElements/showAllPrices/sidebar change (user toggles)
//   - config bar state changes (user interaction)
//
// It NEVER re-renders from depth data updates (50ms tick).
// Each DepthCard handles its own updates independently via events.
// ─────────────────────────────────────────────────────────────────────
const OriginalLayout = memo(({
    visibleElements,
    monitoredTokens,
    depthEvents,
    depthDataRef,
    ...
    const [config, setConfig] = useState(() => {
        const saved = localStorage.getItem('mt_config_original');
    ...
    // --- Logs Resize State ---
    const [logsHeight, setLogsHeight] = useState(350);
    const logsHeightRef = useRef(logsHeight);
    logsHeightRef.current = logsHeight;

    useEffect(() => { setLogsHeight(350); }, [visibleElements.logs]);

    const handleResizeStart = useCallback((e) => {
        e.preventDefault();
        const startY = e.pageY;
        const startHeight = logsHeightRef.current;

        const onPointerMove = (moveEvent) => {
            const delta = startY - moveEvent.pageY;
            setLogsHeight(Math.min(window.innerHeight * 0.8, Math.max(255, startHeight + delta)));
        };
        const onPointerUp = () => {
    ...
    // --- Persistence ---
    useEffect(() => {
        localStorage.setItem('mt_config_original', JSON.stringify(config));
    }, [config]);
    ...
    const handleAdd = () => {
        ...
        config.selectedStrikes.forEach(strike => {
            ...
            const findMatch = (type) => contractsData.find(c =>
                c.s === searchIndex &&
    ...
    const handleUpdateStrike = useCallback((id, newStrike) => {
        const item = monitoredTokensRef.current.find(m => m.id === id);
        if (!item) return;
        ...
        <div className="flex flex-col h-full overflow-hidden space-y-3">
            ...
            {visibleElements.config && (
                <section className={cn(
                    "bg-[#0f1115] border border-white/10 rounded-2xl shadow-xl p-2 flex flex-wrap items-center gap-3 flex-shrink-0 z-20 relative transition-all duration-300",
            ...
            <div className={cn(
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3">
                    {monitoredTokens.map((item) => {
                        if (item.type === 'CE' && !visibleElements.ceDepth) return null;
                        if (item.type === 'PE' && !visibleElements.peDepth) return null;
                        ...
                        return (
                            <DepthCard
                                ...
                                isEditorOpen={activeStrikeEditor?.id === item.id}
                                activeStrikeEditor={activeStrikeEditor}
                                setActiveStrikeEditor={setActiveStrikeEditor}
                                onRemoveToken={onRemoveToken}
                                onUpdateTokenQty={onUpdateTokenQty}
                                handleUpdateStrike={handleUpdateStrike}
                            />
                        );
                    })}
                </div>
            ...
            {visibleElements.logs && (
                <LogsTable
                    ...
                    setShowRelativeTimer={setShowRelativeTimer}
                />
            )}
        </div>
    );
